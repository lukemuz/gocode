...
run concurrently via `agent.Parallel`; a third step consumes their outputs
...
- `agent.Parallel` running two `StepFunc[string]` concurrently and
...
- The same `*agent.Client` reused across all three calls; cost-tiering or
...
- `agent.Parallel`, `agent.StepFunc`, `agent.Result`
- `agent.Client.Ask` for single-shot calls (no tool loop)
