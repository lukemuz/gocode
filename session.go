...
// offending ID. Store implementations use this to produce errors that match
// errors.Is(err, ErrSessionNotFound) while still naming the missing ID.
func SessionNotFound(id string) error {
	return fmt.Errorf("%w: %s", ErrSessionNotFound, id)
}

// SessionExists returns an error that wraps ErrSessionExists with the
// offending ID, the create-side counterpart to SessionNotFound.
func SessionExists(id string) error {
	return fmt.Errorf("%w: %s", ErrSessionExists, id)
}

// Clone returns a deep copy of s so neither the caller's original nor
// the stored copy can alias each other. Store implementations use this on
// both write and read to guarantee isolation.
