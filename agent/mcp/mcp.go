...
// and adapts them into ordinary agent.Tool and agent.ToolFunc values so
...
// agent.Toolset containing one ToolBinding per MCP tool. Each binding's
...
func (s *Server) Toolset(ctx context.Context) (agent.Toolset, error) {
	resp, err := s.call(ctx, "tools/list", nil)
	if err != nil {
		return agent.Toolset{}, fmt.Errorf("mcp: list tools: %w", err)
	}
	...
	if err := json.Unmarshal(resp, &payload); err != nil {
		return agent.Toolset{}, fmt.Errorf("mcp: decode tools/list response: %w", err)
	}

	bindings := make([]agent.ToolBinding, 0, len(payload.Tools))
	for _, mt := range payload.Tools {
		t, err := mcpToolToAgent(mt)
		if err != nil {
			return agent.Toolset{}, err
		}
		fn := s.makeToolFunc(mt.Name)
		bindings = append(bindings, agent.ToolBinding{
			Tool: t,
			Func: fn,
			Meta: agent.ToolMetadata{Source: "mcp"},
		})
	}
	return agent.Toolset{Bindings: bindings}, nil
}
...
// mcpToolToAgent converts an MCP wire tool into an agent.Tool.
// The inputSchema from MCP is already JSON Schema, so we pass it through
// as raw bytes rather than re-serialising through agent.InputSchema.
func mcpToolToAgent(mt mcpTool) (agent.Tool, error) {
...
// makeToolFunc returns an agent.ToolFunc that calls the named MCP tool.
func (s *Server) makeToolFunc(toolName string) agent.ToolFunc {
