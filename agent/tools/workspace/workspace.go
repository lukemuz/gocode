...
// Toolset returns all workspace bindings as an agent.Toolset.
func (w *Workspace) Toolset() agent.Toolset {
...
func (w *Workspace) buildBindings() []agent.ToolBinding {
	...
	listDirTool, _ := agent.NewTool(
		...
		agent.Object(
			agent.String("path", `Directory to list, relative to the workspace root. Defaults to "." (the root).`),
		),
	)
	...
	findFilesTool, _ := agent.NewTool(
		...
		agent.Object(
			agent.String("pattern", "Glob pattern matched against file names (e.g. \"*.go\", \"*_test.go\").", agent.Required()),
			agent.String("path", `Directory to search under, relative to the workspace root. Defaults to ".".`),
		),
	)
	...
	searchTextTool, _ := agent.NewTool(
		...
		agent.Object(
			agent.String("pattern", "Regular expression to search for.", agent.Required()),
			agent.String("path", `Directory to search under, relative to the workspace root. Defaults to ".".`),
			agent.String("include", `Optional glob to filter which file names are searched (e.g. "*.go").`),
		),
	)
	...
	readFileTool, _ := agent.NewTool(
		...
		agent.Object(
			...
			agent.Integer("start_line", "First line to include (1-indexed). 0 or omitted means start of file."),
			agent.Integer("end_line", "Last line to include (1-indexed, inclusive). 0 or omitted means end of file."),
		),
	)
	...
	fileInfoTool, _ := agent.NewTool(
		...
		agent.Object(
			agent.String("path", "Path relative to the workspace root.", agent.Required()),
		),
	)

	return []agent.ToolBinding{
		{Tool: listDirTool, Func: agent.TypedToolFunc(w.listDirectory), Meta: meta},
		{Tool: findFilesTool, Func: agent.TypedToolFunc(w.findFiles), Meta: meta},
		{Tool: searchTextTool, Func: agent.TypedToolFunc(w.searchText), Meta: meta},
...
func (w *Workspace) findFiles(_ context.Context, in findFilesInput) (string, error) {
	...
	return agent.JSONResult(matches)
}
