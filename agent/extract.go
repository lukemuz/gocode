...
//
//	agent.Extract(ctx, client, system, history, agent.ExtractParams[MyPlan]{
...
func Extract[T any](
	...
	if params.Description == "" {
		return captured, LoopResult{}, fmt.Errorf("agent: Extract: ExtractParams.Description is required")
	}
	...
	if err != nil {
		return captured, LoopResult{}, fmt.Errorf("agent: Extract: combine tools: %w", err)
	}
	...
	if !submitted {
		return captured, result, fmt.Errorf("agent: Extract: model ended without calling %q", name)
	}
